...
from hyper import update_module_params, get_num_params, Elementwise, replace_hyper_elementwise
...
class Encoder(nn.Module):
	...
	def __init__(self, dim, hidden_dim=64, max_n=8, **kwargs):
		...
		self.hypernet = Hypernet(key_dim=self.max_n, val_dim=self.hidden_dim, encode_val=True)
...
class Hypernet(nn.Module):
	...
	def __init__(self, key_dim, val_dim, encode_val=False):
		super().__init__()
		...
		self.hypernet = nn.Sequential(Elementwise(dim=self.val_dim, bias=True))
