...
class Visualiser:
	...
	def get_data(self, model, agent=0, layer=0):
		if layer == model.gnn_nlayers:
			x_true = model.merge_gnn.get_values("x")[-1]
			batch_true = model.merge_gnn.get_values("batch")[-1]
			perm = model.merge_gnn.get_values("x_perm_idx")[-1]
			x_pred = model.merge_gnn.get_values("xr")[-1]
			batch_pred = model.merge_gnn.get_values("batchr")[-1]
		else:
			x_true = model.merge_gnn.get_values("x")[layer]
			batch_true = model.merge_gnn.get_values("batch")[layer]
			perm = model.merge_gnn.get_values("x_perm_idx")[layer]
			x_pred = model.merge_gnn.get_values("xr")[layer]
			batch_pred = model.merge_gnn.get_values("batchr")[layer]
