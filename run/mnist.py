...
class TFAE(nn.Module):
    ...
    def forward(self, x, b_idx):
        ...
        z = self.set_ae(
            src_in, tgt_in, #src_mask=mask, tgt_mask=mask, memory_mask=mask,
            src_key_padding_mask=pad_mask, tgt_key_padding_mask=pad_mask
        )
        batch = b_idx
...
class SeqAE(nn.Module):
    def __init__(self, max_seq_len):
        ...
        self.set_ae = AutoEncoder(
            dim=64, hidden_dim=max_seq_len * 64, max_n=max_seq_len, pe="freq",
            data_batch=False
        )
...
def main():
    ...
    trainset = torchvision.datasets.MNIST(root='/tmp/datasets', train=True,
                                            download=True, transform=transform)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,
                                              shuffle=True, num_workers=2)

    testset = torchvision.datasets.MNIST(root='/tmp/datasets', train=False,
                                           download=True, transform=transform)
    ...
    val_loss = 0
    for epoch in pbar:
        # Freeze CNN after some epochs
        if epoch >= 30: 
            for p in model.cnn_ae.parameters():
                if epoch % 5 != 0:
